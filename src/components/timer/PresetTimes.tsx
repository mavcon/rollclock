...
export const PresetTimes: React.FC<PresetTimesProps> = ({
  ...
  <div className="grid grid-cols-3 sm:grid-cols-6 gap-1 animate-slide-up mb-2 relative z-50">
    {presetTimes.map((mins) => (
      <button
        key={mins}
        onClick={() => onSelect(mins)}
        disabled={isDisabled}
        className={`w-full py-1.5 px-2 rounded-[4px] text-base sm:text-lg font-medium transition-all ${
          currentMinutes === mins
